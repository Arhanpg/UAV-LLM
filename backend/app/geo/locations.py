...
# ---------------------------------------------------------------------------
# Master location catalog  (name, lat, lon, building_height_m, category, desc)
# Add new real-world places here; they become available to the LLM immediately.
# ---------------------------------------------------------------------------
DHARWAD_LOCATIONS: list[tuple] = [
    # --- Hospitals / medical ------------------------------------------------
    ...
    ("Big Bazaar Hubli",                  15.3641, 75.1230, 16.0, "mall",
     "Big Bazaar, Keshwapur, Hubli"),
    ("Reliance Fresh Dharwad",            15.4568, 75.0130, 8.0,  "commercial",
     "Reliance Fresh, P.B. Road, Dharwad"),
    ("Reliance Fresh Hubli",              15.3662, 75.1260, 8.0,  "commercial",
     "Reliance Fresh, Vidya Nagar, Hubli"),
    # --- Education ----------------------------------------------------------
    ...
    ("Sana Shaheen Independent PU College", 15.4640, 75.0155, 10.0, "education",
     "Sana Shaheen Independent PU College, Dharwad"),
    ("Karnatak College Dharwad",           15.4601, 75.0059, 12.0, "education",
     "Karnatak College, Station Road, Dharwad"),
    # --- Transit ------------------------------------------------------------
    ...
    ("KSRTC Bus Stand Hubli",             15.3551, 75.1321,  7.0, "transit",
     "KSRTC Central Bus Stand, Hubli"),
    # --- Parks / recreation -------------------------------------------------
    ...
    ("KMF Hubli",                         15.3635, 75.1185, 12.0, "warehouse",
     "Karnataka Milk Federation (KMF / Nandini Dairy), Hubli"),
    ("KMF Dharwad",                       15.4555, 75.0065, 10.0, "warehouse",
     "Karnataka Milk Federation (KMF / Nandini Dairy), Dharwad"),
    ("Nandini Dairy Hubli",               15.3650, 75.1200,  8.0, "warehouse",
     "Nandini Dairy outlet, Hubli"),
    ("Mother Dairy Dharwad",              15.4580, 75.0085,  7.0, "commercial",
     "Mother Dairy / HOPCOMS outlet, Dharwad"),
    # --- Govt ---------------------------------------------------------------
...
def load_locations() -> list[tuple]:
    """Load from JSON file if present, otherwise use inline catalog."""
...
def export_locations_json() -> None:
    ...
    rows = [
        {"idx": i, "name": r[0], "lat": r[1], "lon": r[2], "bh": r[3], "cat": r[4],
...
def generate_world(
    ...
    PKG_LABELS = {
        "PHARMA": "Insulin/Meds", "FOOD": "Food Package",
        "ELECTRONICS": "Device", "FLAMMABLE": "Fuel Canister",
        "OXIDIZER": "O2 Cylinder", "CRYOGENIC": "Cryo Sample",
    ...
    if pkg_requests:
        for i, req in enumerate(pkg_requests):
            ...
            desc = req.get("description") or f"{PKG_LABELS.get(kappa, 'Pkg')} - {pu_c.label} to {dl_c.label}"
            packages.append(
                Package(i, pu_idx, dl_idx, weight, kappa, dl, pr,
                        kappa in {"PHARMA", "FOOD", "CRYOGENIC"}, desc)
            )
            city[pu_idx].pickups.append(
                {"req": i, "kappa": kappa, "w": round(weight, 2), "label": PKG_LABELS.get(kappa, "Pkg")})
            city[dl_idx].drops.append(
                {"req": i, "kappa": kappa, "w": round(weight, 2), "label": PKG_LABELS.get(kappa, "Pkg")})
    else:
        ...
        for i in range(n_auto):
            ...
            pr = 1.8 if kappa == "PHARMA" else (1.4 if kappa in HAZARD_CLASSES else 1.0)
            desc = f"{PKG_LABELS.get(kappa, 'Pkg')} - {pu_c.label} to {dl_c.label}"
            packages.append(
                Package(i, pi_idx, di_idx, weight, kappa, dl_t, pr,
                        kappa in {"PHARMA", "FOOD", "CRYOGENIC"}, desc)
            )
            city[pi_idx].pickups.append(
                {"req": i, "kappa": kappa, "w": round(weight, 2), "label": PKG_LABELS.get(kappa, "Pkg")})
            city[di_idx].drops.append(
                {"req": i, "kappa": kappa, "w": round(weight, 2), "label": PKG_LABELS.get(kappa, "Pkg")})
    ...
    for i in range(min(n_gfz, len(tall))):
        ...
        gzones.append(
            GeoZone(lat=gz_lat, lon=gz_lon, x=c.x + ox, y=c.y + oy,
        ...
        nzones.append(
            GeoZone(lat=nz_lat, lon=nz_lon, x=c.x + ox2, y=c.y + oy2,
